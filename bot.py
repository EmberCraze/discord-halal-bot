...
@app_commands.describe(
    first="The first number to add", second="The second number to add"
)
async def add(
    ...
    await interaction.response.send_message(
        f"{first} + {second} = {first + second}", ephemeral=True
    )
...
async def shop(
    interaction: discord.Interaction, action: Literal["Buy", "Sell"], item: str
):
...
async def quran_leaderboard(interaction: discord.Interaction):
    ...
    for index, entry in enumerate(entries):
        # TODO This is broken once you get to here
        if index == 0:
            embed.add_field(
                name="Name",
                value=interaction.guild.get_member(id=entry.user.id).name,
                inline=True,
            )
    ...
    # embed.set_footer(text="").description = "HEJ"
    db.close()
    await interaction.response.send_message(embed=embed)
...
async def update_reading_progress(
    ...
    user, _ = User.get_or_create(id=interaction.user.id, guild_id=interaction.guild.id)
